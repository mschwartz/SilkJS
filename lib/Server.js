...
Server = (function() {
	...
	return {
		...
		addOnStart: function(config) {
			config = Util.isFunction(config) ? { name: 'unnamed', func: config } : config;
			if (started) {
				config.func();
			}
			else {
		...
		onStart: function() {
			...
			forEach(onStartFuncs, function(config) {
		...
		onStop: function() {
			...
			forEach(onStopFuncs, function(func) {
				func();
			});
		...
		onCodeUpdate: function() {
			forEach(onCodeUpdateFuncs, function(func) {
				func();
			});
		},
